...
INSERT INTO validol_internal.fredgraph_info (id) VALUES (1);
...
SELECT
    data.event_dttm,
    FIRST_VALUE(data.mbase) OVER (PARTITION BY data.series_id, data.mbase_partition_value ORDER BY data.event_dttm) AS mbase,
    FIRST_VALUE(data.tdebt) OVER (PARTITION BY data.series_id, data.tdebt_partition_value ORDER BY data.event_dttm) AS tdebt
FROM (
    SELECT
...
SELECT
    ...
    FIRST_VALUE(data.open_price) OVER (PARTITION BY data.series_id, data.open_price_partition_value ORDER BY data.event_dttm) AS open_price,
    FIRST_VALUE(data.high_price) OVER (PARTITION BY data.series_id, data.high_price_partition_value ORDER BY data.event_dttm) AS high_price,
    FIRST_VALUE(data.low_price) OVER (PARTITION BY data.series_id, data.low_price_partition_value ORDER BY data.event_dttm) AS low_price,
    FIRST_VALUE(data.close_price) OVER (PARTITION BY data.series_id, data.close_price_partition_value ORDER BY data.event_dttm) AS close_price
FROM (
    SELECT
        generated_dates.event_dttm,
        generated_dates.series_id,
...
SELECT
    ...
    FIRST_VALUE(data.fs) OVER (PARTITION BY data.series_id, data.fs_partition_value ORDER BY data.event_dttm) AS fs,
    FIRST_VALUE(data.ul) OVER (PARTITION BY data.series_id, data.ul_partition_value ORDER BY data.event_dttm) AS ul,
    FIRST_VALUE(data.us) OVER (PARTITION BY data.series_id, data.us_partition_value ORDER BY data.event_dttm) AS us,
    FIRST_VALUE(data.flq) OVER (PARTITION BY data.series_id, data.flq_partition_value ORDER BY data.event_dttm) AS flq,
    FIRST_VALUE(data.fsq) OVER (PARTITION BY data.series_id, data.fsq_partition_value ORDER BY data.event_dttm) AS fsq,
    FIRST_VALUE(data.ulq) OVER (PARTITION BY data.series_id, data.ulq_partition_value ORDER BY data.event_dttm) AS ulq,
    FIRST_VALUE(data.usq) OVER (PARTITION BY data.series_id, data.usq_partition_value ORDER BY data.event_dttm) AS usq
FROM (
    SELECT
        generated_dates.event_dttm,
        generated_dates.series_id,
...
SELECT
    ...
    FIRST_VALUE(data.ncl) OVER (PARTITION BY data.series_id, data.ncl_partition_value ORDER BY data.event_dttm) AS ncl,
    FIRST_VALUE(data.ncs) OVER (PARTITION BY data.series_id, data.ncs_partition_value ORDER BY data.event_dttm) AS ncs,
    FIRST_VALUE(data.cl) OVER (PARTITION BY data.series_id, data.cl_partition_value ORDER BY data.event_dttm) AS cl,
    FIRST_VALUE(data.cs) OVER (PARTITION BY data.series_id, data.cs_partition_value ORDER BY data.event_dttm) AS cs,
    FIRST_VALUE(data.nrl) OVER (PARTITION BY data.series_id, data.nrl_partition_value ORDER BY data.event_dttm) AS nrl,
    FIRST_VALUE(data.nrs) OVER (PARTITION BY data.series_id, data.nrs_partition_value ORDER BY data.event_dttm) AS nrs,
    FIRST_VALUE(data.x_4l_percent) OVER (PARTITION BY data.series_id, data.x_4l_percent_partition_value ORDER BY data.event_dttm) AS x_4l_percent,
    FIRST_VALUE(data.x_4s_percent) OVER (PARTITION BY data.series_id, data.x_4s_percent_partition_value ORDER BY data.event_dttm) AS x_4s_percent,
    FIRST_VALUE(data.x_8l_percent) OVER (PARTITION BY data.series_id, data.x_8l_percent_partition_value ORDER BY data.event_dttm) AS x_8l_percent,
    FIRST_VALUE(data.x_8s_percent) OVER (PARTITION BY data.series_id, data.x_8s_percent_partition_value ORDER BY data.event_dttm) AS x_8s_percent
FROM (
    SELECT
        generated_dates.event_dttm,
        generated_dates.series_id,
...
SELECT
    ...
    FIRST_VALUE(data.nrl) OVER (PARTITION BY data.series_id, data.nrl_partition_value ORDER BY data.event_dttm) AS nrl,
    FIRST_VALUE(data.nrs) OVER (PARTITION BY data.series_id, data.nrs_partition_value ORDER BY data.event_dttm) AS nrs,
    FIRST_VALUE(data.pmpl) OVER (PARTITION BY data.series_id, data.pmpl_partition_value ORDER BY data.event_dttm) AS pmpl,
    FIRST_VALUE(data.pmps) OVER (PARTITION BY data.series_id, data.pmps_partition_value ORDER BY data.event_dttm) AS pmps,
    FIRST_VALUE(data.sdpl) OVER (PARTITION BY data.series_id, data.sdpl_partition_value ORDER BY data.event_dttm) AS sdpl,
    FIRST_VALUE(data.sdps) OVER (PARTITION BY data.series_id, data.sdps_partition_value ORDER BY data.event_dttm) AS sdps,
    FIRST_VALUE(data.mmpl) OVER (PARTITION BY data.series_id, data.mmpl_partition_value ORDER BY data.event_dttm) AS mmpl,
    FIRST_VALUE(data.mmps) OVER (PARTITION BY data.series_id, data.mmps_partition_value ORDER BY data.event_dttm) AS mmps,
    FIRST_VALUE(data.orpl) OVER (PARTITION BY data.series_id, data.orpl_partition_value ORDER BY data.event_dttm) AS orpl,
    FIRST_VALUE(data.orps) OVER (PARTITION BY data.series_id, data.orps_partition_value ORDER BY data.event_dttm) AS orps,
    FIRST_VALUE(data.x_4gl_percent) OVER (PARTITION BY data.series_id, data.x_4gl_percent_partition_value ORDER BY data.event_dttm) AS x_4gl_percent,
    FIRST_VALUE(data.x_4gs_percent) OVER (PARTITION BY data.series_id, data.x_4gs_percent_partition_value ORDER BY data.event_dttm) AS x_4gs_percent,
    FIRST_VALUE(data.x_8gl_percent) OVER (PARTITION BY data.series_id, data.x_8gl_percent_partition_value ORDER BY data.event_dttm) AS x_8gl_percent,
    FIRST_VALUE(data.x_8gs_percent) OVER (PARTITION BY data.series_id, data.x_8gs_percent_partition_value ORDER BY data.event_dttm) AS x_8gs_percent,
    FIRST_VALUE(data.x_4l_percent) OVER (PARTITION BY data.series_id, data.x_4l_percent_partition_value ORDER BY data.event_dttm) AS x_4l_percent,
    FIRST_VALUE(data.x_4s_percent) OVER (PARTITION BY data.series_id, data.x_4s_percent_partition_value ORDER BY data.event_dttm) AS x_4s_percent,
    FIRST_VALUE(data.x_8l_percent) OVER (PARTITION BY data.series_id, data.x_8l_percent_partition_value ORDER BY data.event_dttm) AS x_8l_percent,
    FIRST_VALUE(data.x_8s_percent) OVER (PARTITION BY data.series_id, data.x_8s_percent_partition_value ORDER BY data.event_dttm) AS x_8s_percent,
    FIRST_VALUE(data.sdp_spr) OVER (PARTITION BY data.series_id, data.sdp_spr_partition_value ORDER BY data.event_dttm) AS sdp_spr,
    FIRST_VALUE(data.mmp_spr) OVER (PARTITION BY data.series_id, data.mmp_spr_partition_value ORDER BY data.event_dttm) AS mmp_spr,
    FIRST_VALUE(data.orp_spr) OVER (PARTITION BY data.series_id, data.orp_spr_partition_value ORDER BY data.event_dttm) AS orp_spr,
    FIRST_VALUE(data.cl) OVER (PARTITION BY data.series_id, data.cl_partition_value ORDER BY data.event_dttm) AS cl,
    FIRST_VALUE(data.cs) OVER (PARTITION BY data.series_id, data.cs_partition_value ORDER BY data.event_dttm) AS cs,
    FIRST_VALUE(data.ncl) OVER (PARTITION BY data.series_id, data.ncl_partition_value ORDER BY data.event_dttm) AS ncl,
    FIRST_VALUE(data.ncs) OVER (PARTITION BY data.series_id, data.ncs_partition_value ORDER BY data.event_dttm) AS ncs
...
SELECT
    ...
    index.report_type,
    FIRST_VALUE(data.oi) OVER (PARTITION BY data.series_id, data.oi_partition_value ORDER BY data.event_dttm) AS oi,
    FIRST_VALUE(data.dipl) OVER (PARTITION BY data.series_id, data.dipl_partition_value ORDER BY data.event_dttm) AS dipl,
    FIRST_VALUE(data.dips) OVER (PARTITION BY data.series_id, data.dips_partition_value ORDER BY data.event_dttm) AS dips,
    FIRST_VALUE(data.dip_spr) OVER (PARTITION BY data.series_id, data.dip_spr_partition_value ORDER BY data.event_dttm) AS dip_spr,
    FIRST_VALUE(data.ampl) OVER (PARTITION BY data.series_id, data.ampl_partition_value ORDER BY data.event_dttm) AS ampl,
    FIRST_VALUE(data.amps) OVER (PARTITION BY data.series_id, data.amps_partition_value ORDER BY data.event_dttm) AS amps,
    FIRST_VALUE(data.amp_spr) OVER (PARTITION BY data.series_id, data.amp_spr_partition_value ORDER BY data.event_dttm) AS amp_spr,
    FIRST_VALUE(data.lmpl) OVER (PARTITION BY data.series_id, data.lmpl_partition_value ORDER BY data.event_dttm) AS lmpl,
    FIRST_VALUE(data.lmps) OVER (PARTITION BY data.series_id, data.lmps_partition_value ORDER BY data.event_dttm) AS lmps,
    FIRST_VALUE(data.lmp_spr) OVER (PARTITION BY data.series_id, data.lmp_spr_partition_value ORDER BY data.event_dttm) AS lmp_spr,
    FIRST_VALUE(data.orpl) OVER (PARTITION BY data.series_id, data.orpl_partition_value ORDER BY data.event_dttm) AS orpl,
    FIRST_VALUE(data.orps) OVER (PARTITION BY data.series_id, data.orps_partition_value ORDER BY data.event_dttm) AS orps,
    FIRST_VALUE(data.orp_spr) OVER (PARTITION BY data.series_id, data.orp_spr_partition_value ORDER BY data.event_dttm) AS orp_spr,
    FIRST_VALUE(data.nrl) OVER (PARTITION BY data.series_id, data.nrl_partition_value ORDER BY data.event_dttm) AS nrl,
